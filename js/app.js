(function () {
	...
	let cardListener = (deck) => {
		deck.querySelectorAll('li').forEach( (card) => {
			card.addEventListener('click', addListener, false);
		});
	};

	// let addListener = (cardHand) => {
	...
	let captureCards = (card, cardList) => {
		//makes sure flipped card isn't re-added and only 2 cards in array
